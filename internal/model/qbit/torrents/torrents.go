...
type ApiTorrentsInfoReq struct {
	...
	Category string `json:"category,omitempty"` // Get torrents with the given category (empty string means "without category"; no "category" parameter means "any category" <- broken until #11748 is resolved). Remember to URL-encode the category name. For example, My category becomes My%20category
	Tag      string `json:"tag,omitempty"`      // Get torrents with the given tag (empty string means "without tag"; no "tag" parameter means "any tag". Remember to URL-encode the category name. For example, My tag becomes My%20tag
...
type ApiTorrentFilesReq struct {
	Hash    string `json:"hash,omitempty"` //The hash of the torrent you want to get the generic properties of
	Indexes string `json:"indexes"`        // The indexes of the files you want to retrieve. indexes can contain multiple values separated by |.
}
...
type ApiTorrentFiles struct {
	...
	Progress     int    `json:"progress"`    // File progress (percentage/100)
...
type ApiTorrentAddReq struct {
	...
	RootFolder         bool    `json:"rootFolder,omitempty"`         // Create the root folder. Possible values are true, false, unset (default)
